...
class AdminRmpreordercapController extends ModuleAdminController
{
    ...
    public function ajaxProcessSave()
    {
        ...
        if ($id_product <= 0) { http_response_code(400); die(json_encode(array('ok'=>false,'error':'Missing product'))); }
        if ($cap < 0) { http_response_code(400); die(json_encode(array('ok'=>false,'error':'Invalid cap value'))); }
